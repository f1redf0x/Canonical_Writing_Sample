...
By the end, devices sitting in the first spoke's private network should be able to communicate with devices on the second spoke's private networks.

# What you need to begin:

* A Hypervisor like VMWare, Xen, or VirtualBox.

* 6 ubuntu virtual machines - each configured with the public addressing schemes held above.
...
For reference on Network Adapters, the author finds the following video instuctive for understanding [VirtualBox network adapters](https://www.youtube.com/watch?v=Fhdxk4bmJCs&t=418s).
...
First update your Ubuntu instances to make sure that their internal package list is up-to-date.
```apt update
```
...
#node1
[Peer]
PublicKey = INSERT-NODE1S-PUBLIC-KEY-HERE
Endpoint = 192.168.10.31:60031
AllowedIPs = 0.0.0.0/0 #OR 10.2.0.0/0 See NOTE BELOW
```
...
```
[Interface]
PrivateKey = INSERT-NODE1-PRIVATE-KEY-HERE
ListenPort = 60031

#hub
[Peer]
PublicKey = INSERT-HUB-PUBLIC-KEY-HERE
...
```ip -br a s dev wg0
```
Now that the interfaces have been configured on Ubuntu, it's time to tell Wireguard to use them. On both Hub and Node1, run the following commands:
...
ListenPort = 60032
...snip...
PublicKey = INSERT-NODE2S-PUBLIC-KEY-HERE
...
[Interface]
ListenPort = 60032
PrivateKey = INSERT-NODE2-PRIVATE-KEY-HERE
...
```ip r get 10.1
```
This command checks the routing table for networks beginning with 10.1. This should be pointed to the wg0 device. Confirm that the networks are pointing to the right adapter. If the private network is pointed to an ethernet adapter, it's likely that you are on the node with that private network. I.E. Node1 will have 10.1.0.0 on it's ethernet interface since it's the host.

At this point, test your client device connections. Despite being on seperate subnets, client1 should be able to ping client2:
```ping 10.2.0.11
```
and client3
```ping 10.3.0.11
...
3. Wireguard should be configured to act as a systemd daemon to allow for easier management and logging.
